...
void DumpInformation (void *);//(FILE *, char[BUFSIZ]);
...
int main(int argc, char * argv[]) {
	...
	buffer->file = "Dataset-Small.pcap";
	buffer->len = 0;
...
void *producer(void *arg) {
	...
	/*  buffer->fp = fopen(buffer->file, "r");
		 parseHeader(buffer->fp);
		 DumpInformation(buffer->fp, buffer->buf);
		 fclose(buffer->fp); */
	DumpInformation(buffer);
	/*while (1) {
